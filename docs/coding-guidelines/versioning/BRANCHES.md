...
Administrador y principal responsable del repositorio. En su función como líder técnico en el proyecto tiene la responsabilidad de garantizar que el gitflow se está siguiendo correctamente. Además es el usuario que da la aprobación para el paso de cambios a la rama master del desarrollo.
...
- Solo el maintainer puede mezclar cambios a esta rama
...
Para asegurar que los cambios solo sean mezclados haciendo uso de pull request, el maintainer puede agregar reglas las cuales se crean en el repositorio por medio del menú settings.
...
- La mezcla de cambios está limitada por la función de Pull request, no se permiten merge. Para la creación de pull request revise [nuestras normas de estilo](../style/PULL_REQUESTS.md)
- En la rama se trabaja una versión de la aplicación a la vez, al pasar a producción la rama debe quedar idéntica a master. No se deben trabajar en dos versiones de la aplicación a la vez.
...
Las ramas de feature son ramas de carácter temporal que están destinadas para albergar el código de un developer o varios, que estén trabajando sobre una funcionalidad o característica (feature) de la aplicación. Estas ramas tienen las siguientes reglas:
...
Las ramas de hotfix son ramas de carácter temporal que se crean para la atención de un error crítico en la aplicación cuando está en producción y que no puede esperar al despliegue de un nuevo release. Estas ramas tienen las siguientes reglas:
...
- Reporte al equipo de desarrollo sobre envío de cambios a develop
...
- El maintainer crea un release de código siguiendo el [versionamiento semántico](VERSIONING.md)
...
- Se organiza con el equipo de despliegue (deployer) los pasos para la puesta en producción
...
- Se reporta que hay un issue en desarrollo por medio del tablero de issues y se asigna al desarrollador y al maintainer. Para reportar issues sobre el producto revise [nuestras normas de estilo](../style/WRITE_BUG.md)
...
- Cuando se resuelve el error, se crea un pull request a master el cual requiere aprobación del tester del equipo y del maintainer. Para la creación de pull request revise [nuestras normas de estilo](PULL_REQUESTS.md)
