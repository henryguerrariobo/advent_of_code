...
8. La versión major X (X.y.z | X > 0) DEBE ser incrementada si cualquier cambio no compatible con la versión anterior es introducida a la _API_. PUEDE incluir cambios de nivel menor y/o patch. Las versiones patch y minor DEBEN ser reseteadas a 0 cuando se incrementa la versión major.
...
Apenas te des cuenta de que rompiste con la especificación de Versionado Semántico, repara el problema y publica una nueva versión minor que corrige el problema y recupera la compatibilidad. Incluso en esta circunstancia, es inaceptable modificar releases versionados. Si corresponde, documenta la versión que rompe la especificación e informa a tus usuarios que estén atentos a ella.
